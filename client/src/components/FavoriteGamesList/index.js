...
const FavoriteGamesList = ({ username, games }) => {
  ...
  if (!games || !games.length) {
    return (
      <p>
        <span className="username-style">{username}</span>, go favorite some
        games!
      </p>
    );
  }
  ...
  return (
    <div>
      <Box sx={{ flexGrow: 1, m: 2 }}>
        ...
        >
          <h5>
            <span className="username-style">{username}'s</span> {games.length}{' '}
            {games.length === 1 ? 'favorite game' : 'favorite games'}
          </h5>
          {games.map((game) => (
